...
(function (global) {
  ...
  function softenCaps(text) {
    return String(text || '').replace(/\b[A-ZÁÉÍÓÚÜÑ]{3,}\b/g, function (w) {
  ...
  /* Palabras que van seguidas de un designador con letra */
  var CLAVES_DESIGNADOR = '(?:calles? de rodaje|taxiways?|pistas?|runways?|umbral(?:es)?|' +
    'thresholds?|plataformas?|aprons?|categor[íi]a|category)';
  var TOKEN_DESIGNADOR = '(?:[A-Z]\\d{1,2}|[A-Z]{1,3})';
  var SEPARADOR = '(?:,|[yY]|[aA][nN][dD])';
  var RE_DESIGNADOR = new RegExp('\\b(' + CLAVES_DESIGNADOR + ')\\s+(' + TOKEN_DESIGNADOR +
    '(?:\\s*' + SEPARADOR + '\\s*' + TOKEN_DESIGNADOR + ')*)', 'g');

  /* "calle de rodaje A4 y B" -> "calle de rodaje Alfa cuatro y Bravo".
     Solo toca las letras que siguen a una de las palabras clave, para no
  ...
  function foneticoDesignadores(text, lang) {
    return String(text || '').replace(RE_DESIGNADOR, function (todo, clave, lista) {
      ...
      return clave + ' ' + convertida;
    });
  }

  /* "B747-8" o "Boeing 747-8" -> "Boeing siete cuatro siete ocho" */
  function foneticoAeronaves(text, lang) {
    var t = String(text || '');
    t = t.replace(/\b(Boeing|Airbus)\s+(\d{3})(?:-(\d{1,2}))?\b/gi, function (m, marca, d3, d2) {
      var nombre = /^b/i.test(marca) ? 'Boeing' : 'Airbus';
      return nombre + ' ' + N.spell(d3, lang) + (d2 ? ' ' + N.spell(d2, lang) : '');
    });
    t = t.replace(/\b([AB])(\d{3})(?:-(\d{1,2}))?\b/g, function (m, l, d3, d2) {
      return (l === 'B' ? 'Boeing' : 'Airbus') + ' ' + N.spell(d3, lang) + (d2 ? ' ' + N.spell(d2, lang) : '');
    });
    return t;
  }

  /* Letra con uno o dos dígitos suelta (A4, B9, H1): siempre es un designador */
  function foneticoSueltos(text, lang) {
    return String(text || '').replace(/\b([A-Z])(\d{1,2})\b/g, function (m, letra, digitos) {
      return N.phonetic(letra, lang) + ' ' + N.spell(digitos, lang);
    });
  }

  /* Conjunciones de una letra: la "Y" en mayúscula la leen como "i griega" */
  function minusculasSueltas(text) {
    return String(text || '').replace(/\b([YOU])\b/g, function (m, c) { return c.toLowerCase(); });
  }

  /* Cadena completa para locutar un texto libre (NOTAM o información adicional) */
  function paraLocutar(text, lang) {
    var t = foneticoAeronaves(text, lang);
    t = foneticoDesignadores(t, lang);
    t = foneticoSueltos(t, lang);
    t = minusculasSueltas(t);
    return speakNumbers(softenCaps(t), lang);
  }
  ...
  function build(lang, obs, cfg) {
    ...
    if (obs.windshear) {
      b.add((L ? 'Cizalladura del viento reportada: ' : 'Wind shear reported: ') + obs.windshear + '.',
        (L ? 'Cizalladura del viento reportada: ' : 'Wind shear reported: ') + paraLocutar(obs.windshear, lang) + '.');
    }
    ...
    if (extra.length) {
      ...
      extra.forEach(function (line, i) {
        ...
        b.addLine(line, paraLocutar(line, lang));
      });
    }
